...
using SpaceEngineersCalculator.Desktop.Helpers;

namespace SpaceEngineersCalculator.Desktop.Caliburn
{
	public abstract class ScreenValidationBase : ScreenValidationImpl
	{
	...
	public abstract class ScreenValidationImpl : Screen, ISupportValidation, ISupportEditTracking, ICloneable
	{
		...
		[field: NonSerialized]
		private readonly WeakEvent<EventHandler<DataErrorsChangedEventArgs>> _errorsChanged = new WeakEvent<EventHandler<DataErrorsChangedEventArgs>>();
		...
		public bool IsModified
		{
			...
			set
			{
				if (_isModified == value)
					return;
				_isModified = value;
				_isModifiedChanged.Invoke(this, value);
				NotifyOfPropertyChange(nameof(IsModified));
			}
		}

		private readonly WeakEvent<EventHandler<bool>> _isModifiedChanged = new WeakEvent<EventHandler<bool>>();
